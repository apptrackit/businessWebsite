  ...
  return (
    <group ref={ringsRef}>
      <mesh rotation={[Math.PI / 2.6, 0, 0]}>
        <torusGeometry args={[1.95, 0.02, 16, 180]} />
      ...
      <mesh rotation={[Math.PI / 2.9, Math.PI / 3.5, 0]}>
        <torusGeometry args={[2.22, 0.017, 16, 160]} />
        <meshStandardMaterial
          color="#7d8cff"
          emissive="#7d8cff"
...
function ArtifactMesh({ accentColor, intensity }: HeroArtifactProps) {
  ...
  useFrame((state, delta) => {
    ...
    const t = state.clock.elapsedTime;
    const floatOffset = Math.sin(t * 0.9) * 0.18;
    const wobble = Math.sin(t * 1.2) * 0.07;

    groupRef.current.rotation.y += delta * 0.55;
    groupRef.current.rotation.x = gsap.utils.interpolate(groupRef.current.rotation.x, pointer.y * 0.2 + wobble, 0.05);
    groupRef.current.position.x = gsap.utils.interpolate(groupRef.current.position.x, pointer.x * 0.36, 0.06);
    groupRef.current.position.y = gsap.utils.interpolate(groupRef.current.position.y, pointer.y * 0.24 + floatOffset, 0.06);

    const scale = animationState.current.scale;
    coreRef.current.scale.setScalar(scale);
    shellRef.current.scale.setScalar(1.35 + Math.sin(t * 1.1) * 0.03);
...
export default function HeroArtifact({ accentColor, intensity }: HeroArtifactProps) {
  return (
    <div className="artifact-canvas" aria-hidden="true">
      <Canvas camera={{ position: [0, 0, 4.3], fov: 50 }} gl={{ antialias: true, alpha: true }}>
        ...
        <Float speed={1.2} rotationIntensity={0.3} floatIntensity={0.7} floatingRange={[-0.28, 0.28]}>
